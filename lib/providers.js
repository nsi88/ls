 ...
 * @apiDefine Provider
 *
 * @apiSuccess {Number} id Provider id
...
/**
 * @api {post} /providers.json Create a new Provider
 ...
 *     * check_token - Check one time token for get key requests
 ...
 * @apiError 403 Provider has no manage_providers flag
...
exports.create = function(params, callback) {
  validateParams(params, function(err) {
    if (err) {
      ...
      console.debug('Insert into providers', { sign_iv: providerP.sign_iv.toString('hex'), sign_key: providerP.sign_key.toString('hex'), crypto_iv: '[hidden]', crypto_key: '[hidden]' });
      ls.providers.query('INSERT INTO providers SET ?', providerP, function(err, result) {
        if (err) {
          ...
          console.debug('Insert into database', providerD);
          ls.database.query('INSERT INTO providers SET ?', providerD, function(err, result) {
            if (err) {
              console.error(err);
              console.debug('Delete from providers', providerD.encryption_key_id);
...
/**
 * @api {delete} /providers/:name.json Delete a Provider
...
exports.destroy = function(name, callback) {
  getRaw(name, function(err, provider) {
    if (err) {
      ...
      async.parallel([
        function(callback) { ls.providers.query('DELETE FROM providers WHERE id = ?', provider.encryption_key_id, callback) },
        function(callback) { ls.database.query('DELETE FROM providers WHERE id = ?', provider.id, callback) }
...
var cache = new Mcache(MCACHE_TTL, MCACHE_GC_TIME, function(name, callback) {
  ...
  ls.database.query('SELECT * FROM providers WHERE name = ?', name, function(err, result) {
    if (err) {
      ...
      ls.providers.query('SELECT * FROM providers WHERE id = ?', provider.encryption_key_id, function(err, result) {
        if (err) {
          console.error(err);
          callback({ code: 500 });
        } else if (!result[0]) {

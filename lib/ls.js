...
function validateParams(params, verifySign, callback) {
  if (!params.provider) {
    return callback({ code: 400, message: 'Provider missing' });
  }
  if (verifySign) {
    providers.get(params.provider, function(err, provider) {
      if (err) {
        ...
        if (!sign.verify(params, provider.sign_iv, provider.sign_key)) {
          callback({ code: 401, message: 'Missing or invalid signature' });
        } else {
          callback();
        }
      }
 ...
 * @return {Error|Object}
 */
function handleParams(req, verifySign, callback) {
  if (req.method == 'POST' || req.method == 'DELETE') {
    parseBody(req, function(err, params) {
      if (err) {
        callback({code: 500, message: err.message });
      } else {
        validateParams(params, verifySign, function(err) {
          if (err) {
            callback(err);
          } else {
 ...
 * @param  {Object} obj    Will be JSON-stringified
...
function dispatchRequest(req, res) {
  ...
  if (matches[1] == 'providers') {
    if (req.method == 'POST' && !matches[2]) {
      handleParams(req, true, function(err, params) {
        if (err) {
